...
if isempty(find_graflab)==0 %if the .m files exists
    ...
    foundit=1; % all ok/exists
end

% find GrafLab.m in D
if foundit==0
    find_graflab_2=dir(fullfile('D:','**','GrafLab.m'));
    if isempty(find_graflab_2)==0 %if the .m files exists
        m_map_path=find_graflab_2.folder;
        addpath(m_map_path)
        foundit=1; % all ok/exists
    end
end
...
% find GrafLab.m in C
if foundit==0
    find_graflab_3=dir(fullfile('C:','**','GrafLab.m'));
    if isempty(find_graflab_3)==0 %if the .m files exists
        m_map_path=find_graflab_3.folder;
        addpath(m_map_path)
        foundit=1; % all ok/exists
    end
end
