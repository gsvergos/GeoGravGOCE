...
foundit=0; %counter/ not exists

% check if th "m_map" folder exists
...
if isempty(find_m_map_function)==0 %if the .m files exists
    ...
    foundit=1; %counter/ all ok
end


if foundit==0
    % check if th "m_map" folder exists in D
    filelist=dir(fullfile('D:','**','m_hatch.m'));
    if isempty(filelist)==0 %if the .m files exists
        m_map_path=filelist.folder
        addpath(m_map_path);
        newStr_path=[m_map_path,'\private'];
        addpath(genpath(newStr_path)); % add the private m_map path
        foundit=1; %counter/ all ok
    end
end
...
if foundit==0
    % check if th "m_map" folder exists in C    
    filelist=dir(fullfile('C:','**','m_hatch.m'));
    if isempty(filelist)==0 %if the .m files exists
        m_map_path=filelist.folder
        addpath(m_map_path);
        newStr_path=[m_map_path,'\private'];
        addpath(genpath(newStr_path)); % add the private m_map path
        foundit=1; %counter/ all ok % all ok
    end
end


% ----- check 2 -----
if ((exist('m_map', 'file')==7) || foundit==1) % 7 — name is a folder.
