function [GG_coords_GRF,stop_process] = single_process_up_2(NOM_data_0,PSO_data_0,counter_NOM_0,counter_PSO_0,path_NOM,path_PSO)
%    GeoGravGOCE project
%    E. Mamagiannou
%    GravLab, AUTh, 26/8/2020

%    ---------------------------------------------------------------------------------------------
%    ---------- Single Processing ( >1 file ) ----------
...
%    GG_coords_GRF{15}=q3   -//-
%    GG_coords_GRF{16}=q4   -//-
%    GG_coords_GRF{17}= Time GPS - PSO data
%
...
[result] = checking_NOM_PSO(NOM_data_0,PSO_data_0); % check the user's data before the further calculations

% initialize variables for speed
result_GG=cell(length(NOM_data_0),1);

if (result==0)
    for i=1:length(NOM_data_0)
        % single processing/ more than 2 files
        [GG_coords_GRF1] = browse_data_2(NOM_data_0{i},counter_NOM_0,PSO_data_0{i},counter_PSO_0,path_NOM,path_PSO); % processing for more than 1 files
        result_GG{i,1}=GG_coords_GRF1; %save the GG_coords output
    end
    stop_process=0; % counter/ all ok/ checking
    ...
    SGG_GRF{4,1}=GG_coords_GRF{17}; % GPS time (NOM data)
